...
public class Tetris {
    /*블록의 최대 크기 -> iScreenDw 보다는 크게 만들어야 함*/
    private static int iScreenDw;        // large enough to cover the largest block
    private static int nBlockTypes;        // number of block types (typically 7) -> 블록의 종류
    private static int nBlockDegrees;    // number of block degrees (typically 4) -> 각도
    ...
    public static void init(int[][][][] setOfBlockArrays) throws Exception { // initialize static variables
        nBlockTypes = setOfBlockArrays.length; // 블록 종류의 갯수 : 7
        nBlockDegrees = setOfBlockArrays[0].length; // 블록 각도의 갯수 : 4
        setOfBlockObjects = createSetOfBlocks(setOfBlockArrays); // set 블록 배열
        iScreenDw = findLargestBlockSize(setOfBlockArrays); // 최대 크기 잡기
    }
